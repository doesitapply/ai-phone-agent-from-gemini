...
10. A controlled inbox test remains visible to the controller after
    preparation. It advances exactly one seed through draft review, a separately
    confirmed one-mailbox provider request, provider reconciliation, immutable
    mailbox inspection, and five-item finalization before another test can be
    prepared. Only the send step reports
    `executionEffect: one_controlled_seed_email`.
11. The controller will not advertise recipient review or execution until the
...
12. A full operator separately submits one approved email only when both the
...
13. Signed delivery, bounce, complaint, suppression, and reply events become
   measured facts.
14. One operator-confirmed callback can return one fact to Velvet.
15. Recorded outcomes become evaluation data. A human-approved message
   candidate still requires a second full-operator release before it becomes
   the control for a future experiment.

Velvet acquisition-experiment exports now use one stable campaign binding per
immutable sourcing experiment. Separate request, assignment, batch, lead, and
outcome receipts remain intact, but all controlled arms arrive in one SMIRK
campaign. This is the required bridge between source selection and SMIRK's
campaign-scoped message experiments; ordinary and released-policy batches keep
their existing request-scoped campaign identities.
...
| Velvet persistence | `DATABASE_URL=<loopback disposable MySQL> pnpm test:smirk:persistence` passes three tests | Bounded Maps + owner-email quote receipts, verified-email provenance, batch reservation, signed outcomes, workspace isolation, candidate creation, human approval, and one learned zero-spend batch persist correctly | Real Maps, Hunter, email delivery, SMS, telephony, production migration, or commercial results |
| Cross-system local persistence | `npm run -s check:velvet-smirk:persistence` creates fresh disposable MySQL and Postgres databases, runs actual local HTTP routes, traps production network access, intercepts the advisory-QC, Maps, Hunter, email-delivery, and received-email GET adapters, and drops both databases | The read-only Velvet runtime preflight verifies all required tables plus one separate admin-owned research and outcome key. Two authenticated connection-proof requests prove exact dedicated scopes, one admin owner, distinct credentials, one workspace, a matching HMAC secret, and unchanged API-key usage state. The preparer proof rejects a forged key, blocks on an unreviewed positive interaction, persists one review-only row plus one immutable event, returns the same receipt on replay, makes zero integration requests, and is cancelled locally before the experiment continues. The discovery proof performs two intercepted Maps requests plus one intercepted Hunter request for the ordinary lead, and 44 Maps plus 40 Hunter requests for the balanced 40-lead experiment; the real executor persists verified owner-email provenance under exact line-item and combined caps, with no manual email insertion. The run also proves one ordinary reviewed prospect loop plus a frozen two-arm sourcing experiment: one forged definition binding is rejected, four exact balanced assignments create and import 40 synthetic leads into one immutable experiment-bound SMIRK campaign, JSONB status-key reordering cannot break either the schema-normalized assignment receipt or the canonical evidence hash, missing outcome coverage blocks closure, 40 signed synthetic outcomes plus one replay remain idempotent, and closure records only a statistically eligible `hvac` recommendation. Closure itself creates no candidate and changes no policy. The exact reviewed result is then proposed once, exact proposal replay is idempotent, release before the separate approval is rejected, approval alone changes no policy, and a separately hash-bound release affects only future research criteria. Exact release replay is idempotent. One later `latest_released` request is prepared with the released `hvac` filter, remains unexecuted with zero provider requests, and changes no existing batch, contact, or spend authority. Twenty of the same Velvet-fed leads then enter a frozen 10/10 manual-call copy cohort, produce only review drafts, receive synthetic historical outcomes without a call, return 20 signed feedback receipts to Velvet, and close on complete evidence. The candidate remains inert until a separate approval and exact policy release; the winner changes only the next experiment control, leaves all existing jobs unchanged, rejects the old control, and binds the remaining 20 untouched leads to a second prepared cohort. The same stable campaign then completes a synthetic five-inbox gate through five advisory receipts, five separate approvals, five intercepted single-recipient provider requests, five immutable primary-folder and SPF/DKIM/DMARC PASS inspections, and one campaign-and-variant-bound PASS receipt. That exact receipt activates a frozen 10/10 email experiment and prepares 20 idempotent review-only drafts with zero cohort execution. The ordinary loop proves deterministic and advisory QC receipts, receipt tamper rejection, separate human approvals, a synthetic manual-call record, one intercepted email-provider acceptance, signed delivery, one intercepted bounded received-email GET, a replay-safe plain-text content receipt, exact human reply classification, workspace isolation, and stable canonical `replied` state | Production token values, a live deploy, an actual registry lookup, legal authorization to contact, an actual model, Maps or Hunter request, phone call, external email, real mailbox placement, delivery or reply, paid provider request, customer response, causal market lift, or revenue |
...
| SMIRK revenue-loop controller | `GET /api/prospecting/revenue-loop` is exercised inside `npm run -s check:velvet-smirk:persistence` after the durable cross-system loop completes | One workspace-scoped read reports exact campaigns, qualified leads, every active controlled-inbox seed state, channel-specific unresolved outcomes, mandatory advisory-QC readiness, signed email-webhook readiness, exact inbound-content retrieval readiness, callback state, immutable guardrails, and one next safe step without exposing credentials or taking an external action. It labels a controlled seed provider request as `one_controlled_seed_email`, refuses to collapse preparation into sending, and refuses to advertise prospect email execution when either signed measurement or receipt-bound reply review is unavailable | Approval, execution, background automation, deployed parity, provider availability, contact, or revenue |
...
| SMIRK inbox-placement persistence | `npm run -s check:prospect-inbox-placement:persistence` creates a disposable Postgres database, prepares exactly five allowlisted controlled seed jobs, uses the ordinary single-recipient approval and execution contracts with a fake Resend transport, and queries the real revenue-loop route after every persisted transition. It proves exact `REVIEW -> SEND_ONE -> INSPECT` focus for each seed, then `FINALIZE -> PREPARE_EMAIL_EXPERIMENT`, records five immutable inspections, finalizes one PASS receipt, binds it to one matching email experiment, rejects a seed outcome at the write boundary, confirms zero market outcomes and zero Velvet callbacks, rejects replay drift, and drops the database | The 2/2/1 provider contract, hidden seed records, one-controlled-email execution label, immutable folder/authentication receipts, exact controller focus, exact variant binding, experiment activation gate, workspace isolation, and seed isolation persist without network access | Actual mailbox placement, DNS authentication, real provider acceptance, deployed migration, contact, or revenue |
...
`smirk-velvet.discovery-request.v2` lets SMIRK request a deterministic Maps
plus owner-email quote for one category and metro. It carries an opaque request ID, workspace ID,
...
Velvet limits the combined quote to 20 leads and 500 cents. The quote records
separate exact request, unit-cost, and maximum-cost line items for Maps and
Hunter, plus totals that must equal their line-item sums. A privileged Velvet browser
...
executor creates public-source research records and may persist one same-domain,
valid Hunter owner email per new lead. It cannot call an email-delivery, SMS,
LLM, or telephony provider, and enrichment grants no contact authority.

`velvet-smirk.discovery-status.v2` returns immutable request and quote hashes,
...
PROSPECT_EMAIL_RECEIVING_WORKSPACE_ID
```
...
mailbox. Every message stops at `REVIEW_REQUIRED`; the immutable receipt stores
zero, one, or two recent outreach candidates without claiming a positive
outcome. A full operator must retrieve and inspect the exact message before
classifying it as reply, verified opt-out, or not actionable. The content path
is explicit: `POST
/api/prospecting/email-replies/:reviewId/content` requires the immutable review
hash, an exact confirmation, and no-contact/no-send attestations. It performs
one bounded Resend `GET /emails/receiving/:id`, verifies the provider ID,
sender, receiving address, workspace, and plain-text size, then stores only
normalized plain text and an immutable content receipt. HTML, raw MIME,
headers, and attachments are discarded. Replay returns the stored receipt
without another provider request. Classification remains locked until its
request contains that exact receipt hash. Positive reply
attribution requires an exact candidate. A verified unmatched opt-out still
creates recipient suppression, but it cannot fabricate a prospect DNC outcome;
a matched opt-out atomically records both. Resolution never sends, follows up,
or authorizes contact. The receiving credential must be distinct from both
Resend send keys and all SMIRK operator/integration keys. The application path
is GET-only; provider-side permission scope still requires deployment-time
inspection and must not be described as proven read-only without provider
evidence.
...
the single email-provider adapter request and single received-email GET are
captured in memory, and any other
...
then records a signed delivery webhook, stages a signed inbound email, retrieves
one exact plain-text content receipt with replay deduplication, resolves the
exact immutable reply candidate, dispatches all
three resulting outcome callbacks out of order, verifies exact replay, and
confirms both stores retain the canonical `replied` state with cross-workspace
denial. The same run
activates one frozen two-arm sourcing experiment, rejects a changed definition
hash, persists one control and one challenger assignment, imports ten synthetic
leads per arm, rejects closure before all outcomes exist, records twenty signed
synthetic outcomes, verifies one exact outcome replay, and closes to a
recommendation-only result without creating a candidate or changing runtime
policy. The proof then proposes the exact reviewed result, verifies proposal
replay, blocks release before approval, records a separate evidence-only
approval, and applies one hash-bound future-research-only release. Release
replay is idempotent, and one later `latest_released` request is narrowed to
`hvac` but remains `PREPARED`, unexecuted, and zero-provider-request. It also
calls
