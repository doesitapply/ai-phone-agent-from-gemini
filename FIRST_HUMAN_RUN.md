...
- Obtain a separate target-specific approval naming the exact E.164 number privately: `APPROVE_SMIRK_REAL_PROOF_CALL: <exact-approved-e164>`.
...
- The guarded proof runner re-runs `check:pre-proof-call-live` and stops before dialing unless the deployed app passes the non-mutating live safety audit and both exact confirmations pass after readiness. Readiness alone never authorizes a call. `check:post-deploy-live` still verifies proof artifacts, post-call intelligence, and dashboard proof freshness after a call.
