...
test("legacy synthetic handoff payloads normalize into the evidence inbox", async () => {
  let received: any;
  const handler = createVelvetAcquisitionHandler({
    dbEnabled: true,
    isSchemaReady: () => true,
    ...
    store: {
      async receive(input) {
        ...
        return {
          outcome: "created",
          receiptId: "ace_3333333333333333333333333333333333333333",
          acquisitionId: "acq_4444444444444444444444444444444444444444",
          recordKind: "synthetic",
          contactPermission: "not_permitted",
          contactBasis: "synthetic_fixture",
        };
  ...
  await handler({
    ...
    body: {
      workspaceId: 42,
      externalId: "velvet-manus-fake-lead-legacy-0001",
      caller: validSyntheticPayload.caller,
      companyName: validSyntheticPayload.companyName,
      reason: validSyntheticPayload.reason,
      urgency: "low",
    },
  } as unknown as Request, response, () => undefined);
  assert.equal(state.statusCode, 201);
  assert.equal(received.recordKind, "synthetic");
  assert.equal(received.sourceRecordId, "velvet-manus-fake-lead-legacy-0001");
  assert.match(received.sourceEventId, /^velvet-manus-fake-legacy-event-/);
  assert.notEqual(received.sourceEventId, received.sourceRecordId);
});
