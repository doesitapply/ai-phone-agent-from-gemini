...
export const TOOL_DECLARATIONS = [
  ...
  },
  {
    ...
    description:
      "Create a tracked SMIRK client onboarding intake for a business that wants SMIRK, a trusted employee calling in a new client, or a direct caller asking to set up missed-call recovery. Use after you have at least the business name and one reliable contact method. This creates the owner's setup task and records the published recurring-checkout path; it does not take card details or charge the caller.",
    parameters: {
      type: Type.OBJECT,
      properties: {
        ...
        business_phone: { type: Type.STRING, description: "Business phone number they want SMIRK to help protect" },
        ...
        plan_interest: { type: Type.STRING, description: "Record starter, pro, or enterprise interest. Starter is the only current first-dollar checkout; broader plans require owner review." },
        onboarding_notes: { type: Type.STRING, description: "Any other setup details, trust concerns, requested workflow, CRM notes, call-routing needs, or buyer objections" },
  ...
  },
  {
    ...
    parameters: {
      ...
      properties: {
        preferred_time: {
          type: Type.STRING,
          description: "Preferred callback time or time window in the caller's words, such as 'today after 3pm'.",
        },
        reason: {
          type: Type.STRING,
