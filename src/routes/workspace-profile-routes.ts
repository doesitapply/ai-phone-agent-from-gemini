...
export function registerWorkspaceProfileRoutes(app: Express, deps: WorkspaceProfileRouteDeps): void {
  ...
  app.post("/api/workspace/generate-prompt", dashboardAuth, async (req: Request, res: Response) => {
    try {
      if (!dbEnabled) return res.status(503).json({ error: "Prompt generation requires a connected workspace with durable storage.", code: "DURABLE_STORAGE_UNAVAILABLE" });
  ...
  app.post("/api/workspace/website-scan", dashboardAuth, async (req: Request, res: Response) => {
    try {
      if (!dbEnabled) return res.status(503).json({ ok: false, error: "Website intake requires a connected workspace with durable storage.", code: "DURABLE_STORAGE_UNAVAILABLE" });
      const wsId = getWorkspaceId(req);
      const workspaceAuth = (req as Request & { workspaceAuth?: { id?: number } }).workspaceAuth;
      const id = workspaceAuth?.id ?? wsId;
      const workspace = await getWorkspaceById(id);
  ...
  app.post("/api/workspace/greeting-preview", dashboardAuth, async (req: Request, res: Response) => {
    try {
      if (!dbEnabled) return res.status(503).json({ ok: false, error: "Greeting preview requires a connected workspace with durable storage.", code: "DURABLE_STORAGE_UNAVAILABLE" });
      const wsId = getWorkspaceId(req);
      const workspaceAuth = (req as Request & { workspaceAuth?: { id?: number } }).workspaceAuth;
      const id = workspaceAuth?.id ?? wsId;
      const workspace = await getWorkspaceById(id);
  ...
  app.post("/api/workspace/provision-number", dashboardAuth, async (req: Request, res: Response) => {
    try {
      if (!dbEnabled) return res.status(503).json({ error: "Phone provisioning requires a connected workspace with durable storage.", code: "DURABLE_STORAGE_UNAVAILABLE" });
      const wsId = getWorkspaceId(req);
      const workspaceAuth = (req as Request & { workspaceAuth?: { id?: number } }).workspaceAuth;
      const id = workspaceAuth?.id ?? wsId;
      const workspace = await getWorkspaceById(id);
  ...
  app.get("/api/workspace/profile", dashboardAuth, async (req: Request, res: Response) => {
    try {
      if (!dbEnabled) return res.status(503).json({ error: "Live workspace profile is unavailable because durable storage is not connected.", code: "DURABLE_STORAGE_UNAVAILABLE" });
      const wsId = getWorkspaceId(req);
      const workspaceAuth = (req as Request & { workspaceAuth?: { id?: number } }).workspaceAuth;
      const id = workspaceAuth?.id ?? wsId;
      const workspace = await getWorkspaceById(id);
  ...
  app.patch("/api/workspace/profile", dashboardAuth, async (req: Request, res: Response) => {
    try {
      if (!dbEnabled) return res.status(503).json({ error: "Workspace updates require durable storage.", code: "DURABLE_STORAGE_UNAVAILABLE" });
      const wsId = getWorkspaceId(req);
      const workspaceAuth = (req as Request & { workspaceAuth?: { id?: number } }).workspaceAuth;
      const id = workspaceAuth?.id ?? wsId;
  ...
  app.post("/api/workspace/complete-setup", dashboardAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!dbEnabled) return res.status(503).json({ ok: false, error: "Setup completion requires durable storage.", code: "DURABLE_STORAGE_UNAVAILABLE" });
