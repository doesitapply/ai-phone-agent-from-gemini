  ...
  try {
    ...
    if (name === "get_status") {
      const [onCallRows, briefingRows, recentCalls] = await Promise.all([
        ...
        ` as unknown as Promise<{ name: string; role: string }[]>),
        (sql`
        ...
        ` as unknown as Promise<{ content: string; category: string; expires_at: string | null }[]>),
        (sql`
          SELECT direction, duration FROM calls
          WHERE workspace_id = ${wsId}
          ORDER BY created_at DESC LIMIT 3
        ` as unknown as Promise<{ direction: string; duration: number }[]>),
      ]);
...
export function registerBossModeRoutes(app: Express, dashboardAuth: RequestHandler, requireOperator: RequestHandler, dbEnabled: boolean): void {
  ...
  router.post("/settings", dashboardAuth, requireOperator, async (req: Request, res: Response) => {
    const wsId = (req as any).workspaceId ?? 1;
  ...
  router.post("/context", dashboardAuth, requireOperator, async (req: Request, res: Response) => {
    const wsId = (req as any).workspaceId ?? 1;
  ...
  router.get("/audit", dashboardAuth, requireOperator, async (req: Request, res: Response) => {
    const wsId = (req as any).workspaceId ?? 1;
