...
const specs = [
  ...
  { label: 'Stripe enterprise link ID', keys: ['STRIPE_PAYMENT_LINK_ENTERPRISE_ID'], note: 'exact webhook/product binding', required: true, live: true },
  { label: 'Stripe secret key', keys: ['STRIPE_SECRET_KEY'], note: 'hosted checkout session creation', required: false, live: true },
  { label: 'Stripe restricted revenue key', keys: ['STRIPE_REVENUE_READ_KEY'], note: 'read-only settled revenue proof', required: false, live: true },
