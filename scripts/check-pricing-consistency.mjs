...
expect(pricingApiHas('features: ["Smart voicemail", "Existing-number forwarding", "Lead capture", "Owner email alerts", "Callback task queue", "Proof dashboard"]') || pricingApiHas("features: ['Smart voicemail', 'Existing-number forwarding', 'Lead capture', 'Owner email alerts', 'Callback task queue', 'Proof dashboard']"), 'starter features still imply texting or are out of sync');
...
expect(liveBuyerRoutes.includes("body?.status_label === 'No activation request found'"), 'live buyer route audit must expect public checkout-status not-found label');
...
expect(buyerRoutes.includes('app.post("/api/checkout/create", publicDemoRateLimit') && buyerRoutes.includes('res.setHeader("Cache-Control", "no-store")'), 'checkout create route must disable response caching');
...
expect(appHas('The owner email will receive workspace access or the next activation step.'), 'public success page overpromises or omits owner-email activation handoff');
expect(appHas('Payment captured') && appHas('Activation queued') && !appHas('Workspace access sent'), 'public success page must not claim workspace access was already sent');
