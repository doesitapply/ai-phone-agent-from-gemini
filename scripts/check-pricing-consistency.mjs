...
expect(standalonePricingPage.includes('startCheckout(plan, { businessName, ownerEmail, ownerPhone, termsAccepted })'), 'standalone pricing must pass buyer identity and policy acknowledgement into checkout metadata');
expect(standalonePricingPage.includes('One plan you can buy today.') && standalonePricingPage.includes('Starter is missed-call recovery') && !standalonePricingPage.includes('Basic keeps owners focused'), 'standalone pricing hero must present Starter as the single public offer');
...
expect(liveBuyerRoutes.includes("availability.starter !== true") && liveBuyerRoutes.includes("plans.some((plan) => plan.id !== 'starter')") && liveBuyerRoutes.includes("pricingCheckoutAvailability = { starter: true, pro: false, enterprise: false }"), 'live buyer route audit must require Starter as the only public plan and the sole available first-dollar checkout');
