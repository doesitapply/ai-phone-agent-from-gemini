...
const checkoutActivationStatus = fs.readFileSync("src/checkout-activation-status.ts", "utf8");
...
expect("proof-call execution is bound to the exact customer request and workspace", workspaceActivationRoutes.includes("SMIRK_PROOF_WORKSPACE_ID=${id} SMIRK_PROOF_REQUEST_ID=${event.id}"));
...
expect("stripe webhook keeps only the explicitly labeled synthetic smoke bypass", checkoutSafety.includes("export function isApprovedSyntheticPaidHandoffSmoke") && checkoutSafety.includes('startsWith("evt_smirk_paid_handoff_")') && checkoutSafety.includes('metadata.source === "gate3-stripe-webhook-smoke"') && buyerRoutes.includes("isApprovedSyntheticPaidHandoffSmoke(event)"));
expect("stripe webhook acknowledges unrelated Checkout products outside SMIRK rescue", buyerRoutes.includes("configuredPaymentLinkCandidates") && buyerRoutes.includes("paymentLinkProof.ignored") && buyerRoutes.includes("acknowledged outside the configured SMIRK fulfillment boundary"));
...
expect("paid checkout exposes manual telephony as an incomplete activation state", saas.includes('status: "PENDING_MANUAL_TELEPHONY"') && saas.includes('event_type: "telephony_provisioning_required"') && provisioningRoutes.includes('PENDING_MANUAL_TELEPHONY: "Workspace ready; phone line setup remains manual"'));
expect("buyer invite resend preserves incomplete telephony state", saas.includes("checkoutProvisioningStatusAfterInviteDelivery") && checkoutActivationStatus.includes('? "workspace_created"') && checkoutActivationStatus.includes(': "PENDING_MANUAL_TELEPHONY"'));
...
expect("successful customer number purchase advances one pending checkout activation", workspaceProfileRoutes.includes("pendingCheckoutTelephonyRequestId") && workspaceProfileRoutes.includes("advancePendingCheckoutTelephony") && workspaceProfileRoutes.includes("AND status = 'PENDING_MANUAL_TELEPHONY'") && workspaceProfileRoutes.includes("updatedRows.length !== 1") && workspaceProfileRoutes.includes("SET status = 'workspace_created'"));
