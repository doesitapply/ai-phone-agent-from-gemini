...
expect("market validation gate verifies actual current launch assets", packageJson.includes('"check:market-validation-launch": "node scripts/check-market-validation-launch.mjs && npm run -s check:launch-touch-approval-integrity && npm run -s check:launch-asset-provenance && npm run -s check:launch-assets && npm run -s check:launch-protected-assets && npm run -s check:launch-walkthrough"'));
...
expect("launch touch packet cryptographically binds exact outreach approval", launchTouchPacketScript.includes("buildLaunchTouchApproval") && launchTouchApprovalHelper.includes('smirk.outreach-batch-approval.v2') && launchTouchApprovalHelper.includes('APPROVE_SMIRK_OUTREACH_BATCH:') && launchTouchPacketScript.includes('approval_payload_sha256'));
